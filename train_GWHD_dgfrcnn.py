...
class DGFRCNN(LightningModule):
    def __init__(self, n_classes, batchsize, exp, reg_weights):
        ...
        self.InsCls = nn.ModuleList([_InsCls(self.n_classes) for i in range(18)])
        self.InsClsPrime = nn.ModuleList([_InsClsPrime(self.n_classes) for i in range(18)])
    ...
    def training_step(self, batch, batch_idx):
      ...
      if(self.mode == 0):
        ...
        if(self.exp == 'dg'):
          ...
          loss_dict['DA_img_loss'] = self.reg_weights[0]*F.cross_entropy(ImgDA_scores, batch[2].to(device=0))
...
if __name__ == '__main__':
  ...
  train_dataloader = torch.utils.data.DataLoader(tr_dataset, batch_size=4, shuffle=True, collate_fn=collate_fn, num_workers=16, drop_last=True)	
  ...
  detector = DGFRCNN(2, 4, args.exp, args.reg_weights) # Num classes + 1 and batch_size
