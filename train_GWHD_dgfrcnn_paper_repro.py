...
class DGFRCNN(LightningModule):
    def __init__(self, n_classes, batchsize, exp, reg_weights, base_lr=1e-3, weight_decay=5e-4):
        ...
        # Seemakurthy et al. combined paper, Faster R-CNN on GWHD:
        # AdamW, lr=0.001, weight_decay=0.0005, batchsize=2.
    ...
    def configure_optimizers(self):
      ...
      optimizer = torch.optim.AdamW([{'params': self.detector.parameters(), 'lr': self.base_lr, 'weight_decay': self.weight_decay },
...
@torch.no_grad()
def evaluate_map(detector, dataloader, output_dir, max_batches=None):
    """
    Evaluate a trained detector using mAP@50, the main metric reported in the
    combined Seemakurthy et al. paper for object detection.
    ...
    overall_metric = MeanAveragePrecision(
        iou_type="bbox",
        class_metrics=True,
        iou_thresholds=[0.5]
    )
    ...
    for batch_idx, data_sample in enumerate(tqdm(dataloader, desc="mAP@50 evaluation")):
        if max_batches is not None and batch_idx >= max_batches:
            break

        images, boxes_list, domain_labels, orig_imgs = data_sample

        # Torchvision detectors are safest with a list[Tensor].
        images_list = [image.to(device) for image in images]

        preds = detector(images_list)
        ...
        # Move predictions/targets to CPU for TorchMetrics consistency.
        preds_cpu = []
        targets_cpu = []
        ...
        overall_metric.update(preds_cpu, targets_cpu)

        for pred_cpu, target_cpu, domain_label in zip(preds_cpu, targets_cpu, domain_labels):
            domain = int(domain_label.item())
            if domain not in domain_metrics:
                domain_metrics[domain] = MeanAveragePrecision(
                    iou_type="bbox",
                    class_metrics=True,
                    iou_thresholds=[0.5]
                )
            domain_metrics[domain].update([pred_cpu], [target_cpu])

    overall = overall_metric.compute()
    overall_map50 = float(overall["map_50"].item())

    domain_rows = []
    for domain in sorted(domain_metrics.keys()):
        metrics = domain_metrics[domain].compute()
        domain_rows.append({
            "domain_index": domain,
    ...
    domain_mean_map50 = float(domain_df["map_50"].mean()) if len(domain_df) else float("nan")

    summary = {
        "overall_map_50": overall_map50,
        "domain_mean_map_50": domain_mean_map50,
        "num_domains": int(len(domain_df)),
    }

    pd.DataFrame([summary]).to_csv(output_dir / "map_summary.csv", index=False)
    domain_df.to_csv(output_dir / "map_per_domain.csv", index=False)

    print("\nmAP@50 evaluation results")
    print("=========================")
    print(domain_df.to_string(index=False))
    print("")
    print(f"Overall mAP@50: {overall_map50:.6f}")
    print(f"Domain-mean mAP@50: {domain_mean_map50:.6f}")
    print(f"Saved per-domain results to: {output_dir / 'map_per_domain.csv'}")
    print(f"Saved summary to: {output_dir / 'map_summary.csv'}")
...
if __name__ == '__main__':
  ...
  train_dataloader = torch.utils.data.DataLoader(tr_dataset, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn, num_workers=args.num_workers, drop_last=True)	
  ...
  detector = DGFRCNN(2, args.batch_size, args.exp, args.reg_weights, base_lr=args.lr, weight_decay=args.weight_decay) # Num classes + 1 and batch_size
  
  # Paper-style mAP@50 evaluation mode
  if args.eval_map:
    ckpt_path = os.path.join(NET_FOLDER, weights_file + '.ckpt')

    if not os.path.exists(ckpt_path):
      raise FileNotFoundError(f"Could not find checkpoint for mAP evaluation: {ckpt_path}")

    print(f"Loading checkpoint for mAP evaluation: {ckpt_path}")
    ...
    if args.map_output_dir is not None:
      output_dir = args.map_output_dir
    else:
      output_dir = os.path.join(NET_FOLDER, f"map_{split_name}_{weights_file}")

    evaluate_map(
      detector=detector,
      dataloader=eval_dataloader,
      output_dir=output_dir,
      max_batches=args.map_max_batches
    )
  ...
  #    logger=False
  #)
  trainer = Trainer(accelerator="gpu", max_epochs=args.max_epochs, deterministic=False, callbacks=[checkpoint_callback, early_stop_callback], num_sanity_val_steps=2)
