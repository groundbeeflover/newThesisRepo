...
RUN_NAME="gwhd_dgfrcnn_paper_${EXP_NUM}"
WEIGHTS_FILE="dgfrcnn_paper_${EXP_NUM}"
...
echo "Starting paper-aligned Faster R-CNN DG run..."
...
if [ "${EVAL_AFTER}" = "map" ] || [ "${EVAL_AFTER}" = "both" ]; then
  echo "Starting paper-style mAP@50 test evaluation for Faster R-CNN DG run..."
...
if [ "${EVAL_AFTER}" = "wada" ] || [ "${EVAL_AFTER}" = "both" ]; then
  echo "Starting auxiliary WADA/ADA-style test evaluation for Faster R-CNN DG run..."
