...
cp train_GWHD_dgfrcnn_paper.py "${RUN_DIR}/config_snapshot/"
...
python train_GWHD_dgfrcnn_paper.py \\
  --exp dg \\
...
if [ "${RUN_WADA}" = "wada" ] || [ "${RUN_WADA}" = "--wada" ] || [ "${RUN_WADA}" = "true" ]; then
  echo "Starting WADA test evaluation for DG run..."
...
EOF
  ...
  echo "./run_gwhd_dg.sh ${EXP_NUM} wada"
fi